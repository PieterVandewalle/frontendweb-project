...
Cypress.Commands.add('goToCategories', () => { 
    Cypress.log({
        displayName: 'goToCategories',
      });
    cy.visit('http://localhost:3000/categories');
});


Cypress.Commands.add('goToCreatePost', () => {
    Cypress.log({
      displayName: 'goToCreatePost',
    });
    cy.visit('http://localhost:3000/posts/create');
});

Cypress.Commands.add('goToMyPosts', () => { 
    Cypress.log({
        displayName: 'goToMyPosts',
      });
    cy.visit('http://localhost:3000/my-account/posts');
});
...
Cypress.Commands.add('toggleUserDropdown', () => {
    Cypress.log({
        displayName: 'toggleUserDropdown',
    });
    cy.get('[data-cy=user_dropdown_btn]').click();
});
